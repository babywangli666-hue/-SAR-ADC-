...
%         [~, max_f_full] = max(p_spect_full);
%         hold on;
%         plot(freq_full(max_f_full), f2_dB_full(max_f_full), 'ro', 'MarkerSize', 8);
%         hold off;
%     end
% end
% --- 可选绘图：只绘制全频带频谱（线性横坐标） ---
if waveplot == 1
    figure;
    set(gcf, 'Renderer', 'painters');        % 强制向量渲染，确保复制质量
    plot(freq_full, f2_dB_full, 'b');
    xlabel('Frequency (Hz)'); ylabel('Magnitude (dB)');
    title('全频带频谱 (0 ~ fs/2)', 'Interpreter', 'tex'); % 解决波浪号偏移问题
    grid on;
    % 标注信号峰值
    [~, max_f_full] = max(p_spect_full);
    hold on;
